...
use crate::storage::{
    MemoryCache, Page, StorageBackend, StorageError,
    MessageInfo, MessagePart, MessageRecord, MessageTime, MessageWithParts, MessageRole,
    SessionRecord, TodoItem, TodoPriority, TodoStatus,
    backend::{read, write},
};

/// Generate a session ID
pub fn generate_session_id() -> String {
    format!("ses_{}", &crate::util::generate_uuid().replace('-', "")[..16])
}

/// Generate a message ID
pub fn generate_message_id() -> String {
    format!("msg_{}", &crate::util::generate_uuid().replace('-', "")[..16])
}

/// Generate a part ID
pub fn generate_part_id() -> String {
    format!("part_{}", &crate::util::generate_uuid().replace('-', "")[..16])
}
...
impl SessionStore {
    ...
    pub async fn create(&self, input: SessionCreateInput) -> Result<SessionRecord, StorageError> {
        ...
        let key = ["session", &session.project_id, &format!("{}.json", session.id)];
        write(&*self.backend, &key, &session).await?;
        
        self.cache.set(format!("session:{}", session.id), session.clone());
    ...
    pub async fn get(&self, session_id: &str) -> Result<Option<SessionRecord>, StorageError> {
        if let Some(cached) = self.cache.get::<SessionRecord>(&format!("session:{}", session_id)) {
            ...
            if key.len() >= 3 {
                ...
                if let Some(session) = read::<SessionRecord>(&*self.backend, &key_refs).await? {
                    if session.id == session_id {
                        self.cache.set(format!("session:{}", session_id), session.clone());
    ...
    pub async fn list(&self, options: SessionListOptions) -> Result<Vec<SessionRecord>, StorageError> {
    ...
    pub async fn remove(&self, session_id: &str) -> Result<(), StorageError> {
        if let Some(session) = self.get(session_id).await? {
            let key = ["session", &session.project_id, &format!("{}.json", session_id)];
    ...
    pub async fn set_title(&self, session_id: &str, title: &str) -> Result<SessionRecord, StorageError> {
...
impl MessageStore {
            ...
            if key.len() >= 3 {
                ...
                if let Some(record) = read::<MessageRecord>(&*self.backend, &key_refs).await? {
                    ...
                    let role: MessageRole = serde_json::from_value(
                        record.data.get("role").cloned().unwrap_or(serde_json::json!("user"))
                    ).unwrap_or(MessageRole::User);
                    
                    let agent = record.data.get("agent")
                        .and_then(|v| v.as_str())
                        .map(String::from);
    ...
    pub async fn page(
        ...
        let start_idx = if let Some(cursor) = before {
            all_messages.iter().position(|m| m.info.id == cursor).unwrap_or(all_messages.len())
        } else {
            all_messages.len()
        };
        
        let end_idx = start_idx.saturating_sub(limit);
        let items: Vec<MessageWithParts> = all_messages.into_iter()
            .skip(end_idx)
...
pub async fn get(&self, session_id: &str) -> Result<Vec<TodoItem>, StorageError> {
        ...
        Ok(read::<Vec<TodoItem>>(&*self.backend, &key).await?.unwrap_or_default())
    }
    ...
    /// Add a todo
    pub async fn add(&self, session_id: &str, content: String, priority: TodoPriority) -> Result<TodoItem, StorageError> {
...
mod tests {
    ...
    async fn test_session_create() {
        ...
        let session = store.create(SessionCreateInput {
    ...
    async fn test_session_get() {
        ...
        let created = store.create(SessionCreateInput {
            project_id: "proj_test".to_string(),
            slug: "test-slug".to_string(),
            directory: "/tmp".to_string(),
            ..Default::default()
    ...
    async fn test_session_list() {
        ...
        store.create(SessionCreateInput {
    ...
    async fn test_message_create_and_list() {
        ...
        let info = MessageInfo {
            ...
            agent: None,
            time: MessageTime { created: 0, updated: None },
        };
        ...
        store.create(session_id, info.clone(), parts.clone()).await.unwrap();
    ...
    async fn test_todo_operations() {
        ...
        let todo1 = store.add(session_id, "Task 1".to_string(), TodoPriority::High).await.unwrap();
        let todo2 = store.add(session_id, "Task 2".to_string(), TodoPriority::Low).await.unwrap();
        
        let todos = store.get(session_id).await.unwrap();
        assert_eq!(todos.len(), 2);
        
        store.update_status(session_id, &todo1.id, TodoStatus::Completed).await.unwrap();
